...
class ChainConfig:
    ...
    def __init__(self, chain_id: int, global_config: Dict[str, Any], chain_config: Dict[str, Any]):
        ...
        subgraph_prefix = os.getenv("SUBGRAPH_URL_PREFIX")
        if not subgraph_prefix:
            raise ValueError("Missing SUBGRAPH_URL_PREFIX env var")
        self.SUBGRAPH_URL = subgraph_prefix.rstrip("/") + self._chain["SUBGRAPH_PATH"]
