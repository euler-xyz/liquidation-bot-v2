...
contract Liquidator {
    ...
    function liquidate_single_collateral_with_pyth_oracle(LiquidationParams calldata params, bytes[] calldata pythUpdateData) external payable returns (bool success) {
        bytes[] memory multicallItems = new bytes[](3);
    ...
    function simulate_pyth_update_and_get_account_status(bytes[] calldata pythUpdateData, uint256 pythUpdateFee, address vaultAddress, address accountAddress) external payable returns (uint256 collateralValue, uint256 liabilityValue) {
    ...
    function simulate_pyth_update_and_check_liquidation(bytes[] calldata pythUpdateData, uint256 pythUpdateFee, address vaultAddress, address liquidatorAddress, address borrowerAddress, address collateralAddress) external payable returns (uint256 maxRepay, uint256 seizedCollateral) {
        IEVC.BatchItem[] memory batchItems = new IEVC.BatchItem[](2);
